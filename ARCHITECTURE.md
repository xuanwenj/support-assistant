...
Phase 1 (RAG pipeline) is functional, split into `ingest.py` (multi-format ingestion, metadata-based routing via `data/metadata.json`, chunking, embedding + storage in Chroma) and `retrieval.py` (distance-threshold-filtered retrieval, exposed as a standalone `retrieve_relevant_chunks()` function, plus a single Claude call for generation when run directly). The split means retrieval can be reused by the Phase 2 router without re-running ingestion on import. No agent framework wraps it (see architecture status update in Section 2). Next step: Phase 2 — Supabase Postgres for structured data, the structured-query tool, and the Next.js frontend.
